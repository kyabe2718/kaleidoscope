...
struct Tokenizer
{
    ...
    explicit Tokenizer(const char* filename)
        : iss{new std::ifstream{filename, std::ios_base::in}} {}
    explicit Tokenizer(const std::string& filename)
        : iss{new std::ifstream{filename, std::ios_base::in}} {}

    Tokenizer(const Tokenizer&) = delete;

    Tokenizer(Tokenizer&& other) noexcept : iss{other.iss}, curTok{std::move(other.curTok)}
    {
        other.iss = nullptr;
        other.curTok = token::unknown{};
    }

    ~Tokenizer()
    {
        if (auto f = dynamic_cast<std::ifstream*>(iss)) {
            f->close();
            delete f;
        }
    }
